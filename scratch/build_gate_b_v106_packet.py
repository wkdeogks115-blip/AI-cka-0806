...
HEAD_SHA = "a8b73b0b2cb13cc0bbbb9bbbb4bf4b4414ed8d66"
ARTIFACT_SHA256 = "c0996266aca5fe0f9b685b395a896346cf18c09be343ca1ae90bcd854fd61581"
CI_RUN_ID = 31762982874
CI_JOB_ID = 94653044582
CI_TESTS = "21/21 PASS"
...
ROOT = Path("gate_b_v106_packet")
ZIP_PATH = Path("GATE_B_CLEAN_REVIEW_PACKET_V1_0_6.zip")
...
identity = {
    ...
    "candidate": "FACTORY_PACKAGE_INTEGRITY_VERIFIER_V1 v1.0.6-patch-candidate",
...
patch_provenance = {
    ...
    "prior_exact_head": "98e89e8c28aa8d520341191a8412bb269d60c229",
    "prior_artifact_sha256": "a0847299f61a6b8fb63a81571326cd638fbf5a89d7a9390ec77292247088d755",
    "reproduced_defect": {
        "class": "declared control-character fail-closed boundary narrower than implementation",
        "facts": [
            "matching path containing U+007F DELETE returned PASS on the prior candidate",
            "matching path containing U+0085 C1 NEXT LINE returned PASS on the prior candidate",
            "prior predicate rejected only ord(ch) < 32"
        ]
    },
    "current_patch": {
        "predicate": "reject ord(ch) < 32 or 127 <= ord(ch) <= 159",
        "regression_vectors": ["newline", "U+007F", "U+0085", "U+009F"],
        "modes": ["DIRECTORY", "ZIP"]
    },
    "review_instruction": "Reproduce and evaluate independently. Do not treat the prior finding severity or prior verdict as authority."
}
...
contract = f"""# TRUE CLEAN INDEPENDENT RE-REVIEW CONTRACT — PR #{PR} v1.0.6
...
02A_PATCH_PROVENANCE_FACTS.json contains only the minimum defect/patch facts needed to understand the current delta. Reproduce them independently; do not inherit a prior severity or verdict.

Review independently at minimum:
- source identity and exact seven-file scope
- U+007F and C1 control-character fail-closed behavior, including U+0085/U+009F, in directory and ZIP modes
- preservation of C0, malformed/non-object MANIFEST row, scalar, non-canonical, traversal, repeated-slash, colon/Windows-drive/ADS, ZIP directory-entry, symlink and directory-enumeration fail-closed behavior
- regression risk and adequacy of new matching-file tests
...
readme = f"""# GATE B CLEAN RE-REVIEW PACKET — PR #22 v1.0.6
...
manifest = {
    ...
    "packet": "GATE_B_CLEAN_REVIEW_PACKET_PR22_V1_0_6",
    "repo": REPO,
    "pr": PR,
    "base_sha": BASE_SHA,
    "head_sha": HEAD_SHA,
...
Path("GATE_B_CLEAN_REVIEW_PACKET_V1_0_6.sha256.txt").write_text(packet_sha + "  " + ZIP_PATH.name + "\n", encoding="utf-8")
summary = {
...
Path("GATE_B_PACKET_BUILD_RESULT_V1_0_6.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
